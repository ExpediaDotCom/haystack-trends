...
class IntegrationTestSpec extends WordSpec with GivenWhenThen with Matchers with BeforeAndAfterAll with BeforeAndAfterEach with EasyMockSugar {
  ...
  override def beforeEach() {
    ...
    embeddedKafkaCluster.createTopic(INPUT_TOPIC,2,1)
    embeddedKafkaCluster.createTopic(OUTPUT_TOPIC,2,1)
    ...
    STREAMS_CONFIG.put(StreamsConfig.NUM_STREAM_THREADS_CONFIG, "2")
