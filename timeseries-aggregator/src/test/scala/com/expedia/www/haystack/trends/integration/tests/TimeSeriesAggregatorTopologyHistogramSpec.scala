...
class TimeSeriesAggregatorTopologyHistogramSpec extends IntegrationTestSpec {
  ...
  "TimeSeries Aggregator Topology for histogram type metrics" should {
    "consume metricPoints from input topic and aggregate them based on rules" in {
      ...
      val expectedOneMinAggregatedPoints: Int = (MAX_METRIC_POINTS - 1) * 7 // Why one less -> won't be generated for  last (MAX_METRICPOINTS * 60)th second metric point
      val expectedFiveMinAggregatedPoints: Int = (MAX_METRIC_POINTS / 5) * 7
      val expectedFifteenMinAggregatedPoints: Int = (MAX_METRIC_POINTS / 15) * 7
      val expectedOneHourAggregatedPoints: Int = (MAX_METRIC_POINTS / 60) * 7
