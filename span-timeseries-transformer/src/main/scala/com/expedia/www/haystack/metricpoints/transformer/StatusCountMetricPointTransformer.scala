  ...
  override def mapSpan(span: Span): List[MetricPoint] = {
    getErrorField(span) match {
      case Some(errorValue) =>
        ...
        if (errorValue) {
          MetricPoint(FAILURE_METRIC_NAME, MetricType.Gauge, tags, 1, span.getStartTime) :: super.mapSpan(span)
        } else {
          MetricPoint(SUCCESS_METRIC_NAME, MetricType.Gauge, tags, 1, span.getStartTime) :: super.mapSpan(span)
        }
