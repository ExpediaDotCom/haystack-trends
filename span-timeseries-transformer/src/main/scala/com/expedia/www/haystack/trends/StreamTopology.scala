...
class StreamTopology(kafkaConfig: KafkaConfiguration, transformerConfiguration: TransformerConfiguration) extends StateListener
  ...
  private def topology(): Topology = {
    ...
    }.to(kafkaConfig.produceTopic, Produced.`with`(new StringSerde(), new MetricTankSerde(transformerConfiguration.enableMetricPointPeriodReplacement, transformerConfiguration.enableBase64EncodedNames)))
    builder.build()
  }
  ...
  private def mapToMetricPointKeyValue(span: Span): java.util.List[KeyValue[String, MetricPoint]] = {
    generateMetricPoints(transformerConfiguration.blacklistedServices)(MetricPointTransformer.allTransformers)(span, transformerConfiguration.enableMetricPointServiceLevelGeneration)
      .getOrElse(Nil)
      .map {
        metricPoint => new KeyValue(metricPoint.getMetricPointKey(transformerConfiguration.enableMetricPointPeriodReplacement, transformerConfiguration.enableBase64EncodedNames), metricPoint)
      }.asJava
  }
