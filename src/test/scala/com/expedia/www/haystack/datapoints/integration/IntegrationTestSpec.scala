...
class IntegrationTestSpec extends WordSpec with GivenWhenThen with Matchers with BeforeAndAfterAll with BeforeAndAfterEach {
  ...
  protected val INPUT_TOPIC = "datapoints"
  protected val OUTPUT_TOPIC = "aggregateddatapoints"
  ...
  override def beforeEach() {
    ...
    PRODUCER_CONFIG.put(ProducerConfig.VALUE_SERIALIZER_CLASS_CONFIG, DataPointSerde.serializer().getClass)
  ...
  def randomDataPoint(metricName: String,
                      value: Long = Random.nextLong(),
                      timestamp: Long = System.currentTimeMillis()): DataPoint = {
    DataPoint(metricName, MetricType.Metric, Map[String, String](), value, timestamp)
  }
