...
# build docker image
DOCKER_IMAGE_TAG := haystack-span-timeseries-transformers
.PHONY: docker_build
docker_build:
...
run_integration_test: docker_build
	# run service : run haystack-span-timeseries-transformers and join network of dependecy services
	docker run \
		-d \
...
.PHONY: release
REPO := lib/haystack-span-timeseries-transformer
